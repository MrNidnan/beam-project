from .oggutils import parse_comment, parse_pages
import functools
import http.server
import socketserver
import pathlib
...
class Listener():
    ...
    def __init__(self, port=8088, quiet=False, outfile=None, append=False, max_tracks=-1, custom_callback=None):
        self.port = port
        self.quiet = quiet
...
def run(maxtandalength, lastlplaylist):
    if not run.traktorListener:
        run.traktorListener = Listener()
        # !!! blocks, start in a separate thread !!!
        run.traktorListener.start()

run.traktorListener = None
